  ...
  <div class="container mx-auto mt-10 p-6 bg-white dark:bg-gray-800 rounded shadow-md">
    ...
    <form action="{{ route('tasks.update', $task) }}" method="POST">
      ...
      <div class="mb-4">
        ...
        <input type="text" name="name" id="name" value="{{ $task->name }}" class="w-full p-2 border border-gray-300 rounded focus:outline-none focus:ring focus:border-blue-300 dark:bg-gray-700 dark:border-gray-600 dark:focus:border-blue-500" required>
      </div>

      <div class="mb-4">
        <label for="description" class="block text-sm font-medium mb-2">Task Description:</label>
        <textarea name="description" id="description" class="w-full p-2 border border-gray-300 rounded focus:outline-none focus:ring focus:border-blue-300 dark:bg-gray-700 dark:border-gray-600 dark:focus:border-blue-500">{{ $task->description }}</textarea>
      </div>
